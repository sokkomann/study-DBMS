...
/*insert into tbl_zoo(id, zoo_name, zoo_address, zoo_detailed_address)
values(1, '서울랜드', '과천', '123');
insert into tbl_zoo(id, zoo_name, zoo_address, zoo_detailed_address)
values(2, '에버랜드', '용인', '456');*/

insert into tbl_zoo(zoo_name, zoo_address, zoo_detailed_address, zoo_max_capacity)
values('에버랜드','경기도 용인시','12345','10');
insert into tbl_zoo(zoo_name, zoo_address, zoo_detailed_address, zoo_max_capacity)
values('서울랜드','경기도 과천시','55555','5');

select id, zoo_name, zoo_address ,zoo_detailed_address ,zoo_max_capacity 
from tbl_zoo;

/*select * from tbl_zoo;*/	/* 이렇게 쓰는게 편하긴 하지만 연산이 더 추가되어 그냥 다 쓰도록 하자. */
...
/*첫 번째 동물원에 두 마리의 동물*/
insert into tbl_animal (animal_name, animal_type, animal_height, animal_weight, zoo_id)
values('망고','펭귄','73.23','15',1);
insert into tbl_animal (animal_name, animal_type, animal_height, animal_weight, zoo_id)
values('파파야','펭귄','66.8','14.2',1);
/*두 번째 동물원에 한 마리의 동물*/
insert into tbl_animal (animal_name, animal_type, animal_height, animal_weight, zoo_id)
values('삼대','닭','39.6','4.7',2);

/*동물원 중에서 최대 수용 동물 수가 5보다 큰 동물원 조회*/
select id, zoo_name ,zoo_address ,zoo_detailed_address , zoo_max_capacity 
from tbl_zoo
where zoo_max_capacity > 5;
...
values ('김민중',1), ('문진우',1), ('이중헌',2), ('이다빈',2);

/*'이중헌'직원의 전체 정보 조회*/
select company_id ,employee_name ,id 
from tbl_employee
where employee_name = '이다빈';
/*'코리아'회사의 전체 정보 조회*/
...
where company_id = 1;
select * from tbl_employee where company_id = 1;
/*'이다빈'직원 삭제*/
...
/*회원 3명 추가*/
insert into tbl_member (member_name, member_email)
values ('김민중','sokkomann@naver.com'), ('망고','mango@naver.com'), ('참깨','sesame@naver.com');
select * from tbl_member ;
/*1번 -> 2번 좋아요*/
insert into tbl_like (giver_id,taker_id) 
values (1,2);
/*1번 -> 3번 좋아요*/
insert into tbl_like (giver_id,taker_id) 
values (1,3);
...
select * from tbl_like 
where taker_id = 1;
/*이름이 '망고'인 회원을 좋아요한 회원의 이름 조회*/
select * from tbl_member
where member_name = '망고';
...
delete from tbl_like
where giver_id = 1 and taker_id = 2;
/******************************************************************************/
/* join을 해보자 */
...
/*댓글 정보와 작성자 이름 조회*/
select m.member_name '댓글 작성자', r.reply_content '댓글' 
from tbl_member m join tbl_reply r 		/*선행테이블에 1이 가게하자*/
on m.id = r.member_id;
/*댓글이 달린 게시글 제목 조회*/	
select p.post_title, p.post_content, r.reply_content
from tbl_post p join tbl_reply r		/*선행테이블에 1이 가게하자*/
on r.post_id = p.id;
/*이메일이 test5555@gmail.com인 회원이 작성한 게시글 정보와 회원의 이름과 나이 조회*/
select * 
from tbl_member m join tbl_post p
on m.id = p.member_id
where m.member_email = 'test5555@gmail.com';
/*댓글 작성자 이름이 '이순신'인 회원 정보와 댓글 정보 조회*//* from -> join -> on -> where -> select */
select m.*, r.* 
...
join 
(
	select pet_ill_name, max(pet_weight) max_weight 	/*from안에 써서 테이블 취급*/
	from tbl_pet
	group by pet_ill_name 
) p2
